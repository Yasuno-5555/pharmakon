...
use crate::model::{CompletionRequest, CompletionResponse, AgentModel, ToolCall, FunctionCall, AgentResult, AgentError};
use reqwest::Client;
...
impl AgentModel for GroqModel {
    ...
    async fn complete(&self, request: CompletionRequest) -> AgentResult<CompletionResponse> {
        ...
        let content = choice["message"]["content"].as_str().map(|s| pharmakon_common::MessageContent::Text(s.to_string()));
        
        let tool_calls = if let Some(tc) = choice["message"]["tool_calls"].as_array() {
            Some(tc.iter().map(|t| ToolCall {
                id: t["id"].as_str().unwrap_or_default().to_string(),
                r#type: t["type"].as_str().unwrap_or_default().to_string(),
                function: FunctionCall {
                    name: t["function"]["name"].as_str().unwrap_or_default().to_string(),
                    arguments: t["function"]["arguments"].as_str().unwrap_or_default().to_string(),
                }
            }).collect())
        } else {
            None
    ...
    async fn stream_complete(&self, request: CompletionRequest) -> AgentResult<Pin<Box<dyn Stream<Item = AgentResult<String>> + Send>>> {
        ...
        if !response.status().is_success() {
            ...
            return Err(AgentError(format!("Groq streaming API error: {}", error_text)));
        }
            ...
            |(mut byte_stream, mut buffer)| async move {
                ...
                loop {
                    ...
                    match byte_stream.try_next().await {
                        ...
                        Ok(None) => return None,
                        Err(e) => return Some((Err(AgentError(format!("Groq stream error: {}", e))), (byte_stream, buffer))),
                    }
                }
            },
        );
