...
impl InitiativeEngineWorker {
    ...
    pub async fn run_initiative_cycle(&self) -> anyhow::Result<()> {
        ...
        let (model, trajectory_context) = {
            ...
            let recent_steps = agent_lock.trajectory.steps.iter()
                .rev()
                .take(10) // Look at the last 10 steps
                .collect::<Vec<_>>();
                
            let mut context_summary = String::new();
            for step in recent_steps.iter().rev() { // Reverse again to maintain chronological order
                match step {
                    ...
                    crate::trajectory::TrajectoryStep::Action { tool, args, .. } => {
                        ...
                        context_summary.push_str(&format!("Action: {} with args {}\n", tool, arg_str));
                    }
                    ...
                    crate::trajectory::TrajectoryStep::Observation { result, .. } => {
                        ...
                        let truncated = if result.len() > 200 { format!("{}...", &result[..200]) } else { result.clone() };
        ...
        let user_prompt = format!("Recent Context:\n{}\n\nBased on this context, what is the most valuable proactive task you can start in the background?", trajectory_context);
