...
use crate::model::{AgentModel, CompletionRequest, Message, ToolDefinition};
...
impl ModelRouter {
    ...
    pub fn recommend_max_tokens(&self, model_name: &str) -> u64 {
        self.economy
            ...
            .recommend_max_tokens(model_name) as u64
    }
    ...
    pub fn record_model_result(
        ...
        self.economy
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .record_model_result(name, latency_ms, success, is_rate_limit);
    }
    ...
    pub async fn execute_with_fallback(
        ...
        pharmakon_common::agent_types::CompletionResponse,
        Arc<dyn AgentModel>,
    )> {
        ...
        let mut response_result: Option<Result<pharmakon_common::agent_types::CompletionResponse>> =
            None;
        ...
        while response_result.is_none() {
            ...
            response_result = Some(
                match tokio::task::spawn(async move { model_lock.complete(request_clone).await })
                    .await
                {
                    Ok(Ok(resp)) => Ok(resp),
            ...
            match &response_result {
                Some(Ok(res)) => {
                    let is_max_tokens = res
                        .finish_reason
                        .as_ref()
                        .map(|fr| matches!(fr, pharmakon_common::FinishReason::MaxTokens))
                        .unwrap_or(false)
                        || res
                            .content
                            .as_ref()
                            .map(|c| {
                                c.to_string()
                                    .contains("[Model stopped: Max tokens reached]")
                            })
                            .unwrap_or(false);

                    if is_max_tokens {
                        let fallback_list =
                            fallback_models.lock().unwrap_or_else(|e| e.into_inner());
                        if current_fallback_index < fallback_list.len() {
                            let fallback_id = &fallback_list[current_fallback_index];
                            log::warn!(
                                "Output token limit (MAX_TOKENS) for {}. Escalating to fallback: {}",
                                target_model.name(),
                                fallback_id
                            );
                            try_send_event(
                                ...
                                Event::Error {
                                    message: format!(
                                        "Output token limit reached (MAX_TOKENS) for {}. Escalating to fallback: {}",
                                        target_model.name(),
                                        fallback_id
                                    ),
                                },
                                ...
                                crate::providers::registry::ModelRegistry::get_model(fallback_id)
                            {
                                ...
                                request.max_tokens =
                                    Some(self.recommend_max_tokens(target_model.name()) as u32);
                    ...
                    let is_empty = res
                        .content
                        .as_ref()
                        .map(|c| c.to_string().trim().is_empty())
                        .unwrap_or(true)
                        && res.tool_calls.is_none();
                    if is_empty {
                        consecutive_empty_responses += 1;
                        log::warn!(
                            "Empty response from {} (consecutive: {})",
                            target_model.name(),
                        ...
                        if consecutive_empty_responses >= 2 {
                            ...
                            if current_fallback_index < fallback_list.len() {
                                let fallback_id = &fallback_list[current_fallback_index];
                                log::warn!(
                                    "Two empty responses from {}. Switching to fallback: {}",
                                    target_model.name(),
                                    fallback_id
                                );
                                try_send_event(
                                ...
                                if let Some(new_model) =
                                    crate::providers::registry::ModelRegistry::get_model(
                                        fallback_id,
                                    )
                                {
                                    target_model = new_model;
                            ...
                            if used > self.token_budget * 8 / 10 {
                                log::warn!(
                                    "Skipping empty-response retry: budget at {:.0}%",
                                    (used as f64 / self.token_budget as f64) * 100.0
                                );
                                break;
                            }
                ...
                Some(Err(e)) => {
                    ...
                    if is_rate_limit && current_fallback_index < fallback_list.len() {
                        let fallback_id = &fallback_list[current_fallback_index];
                        log::warn!(
                            "Rate limit for {}. Falling back to: {}",
                            target_model.name(),
                            fallback_id
                        );
                        try_send_event(
                    ...
                    return Err(anyhow::anyhow!(
                        "All fallback models exhausted. Final error: {}",
                        e
                    ));
                }
                None => {
                    return Err(anyhow::anyhow!(
                        "[InternalError] Model response loop terminated without setting a result."
                    ));
                }
            }
        }
