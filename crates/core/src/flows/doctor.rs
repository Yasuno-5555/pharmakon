...
impl Doctor {
    pub async fn run_check() -> Result<HealthReport> {
        ...
        let openai_ok =
            env::var("OPENAI_API_KEY").is_ok() || secret_store.get_secret("OPENAI_API_KEY").is_ok();
        let anthropic_ok = env::var("ANTHROPIC_API_KEY").is_ok()
            || secret_store.get_secret("ANTHROPIC_API_KEY").is_ok();
        let gemini_ok =
            env::var("GEMINI_API_KEY").is_ok() || secret_store.get_secret("GEMINI_API_KEY").is_ok();

        let telegram_ok = env::var("TELEGRAM_BOT_TOKEN").is_ok()
            || secret_store.get_secret("TELEGRAM_BOT_TOKEN").is_ok();
        let discord_ok_token = env::var("DISCORD_BOT_TOKEN").is_ok()
            || secret_store.get_secret("DISCORD_BOT_TOKEN").is_ok();
        let slack_ok_token = env::var("SLACK_BOT_TOKEN").is_ok()
            || secret_store.get_secret("SLACK_BOT_TOKEN").is_ok();
    ...
    pub fn repair() -> Result<RepairReport> {
        ...
        let home = match dirs::home_dir() {
            ...
            None => {
                report
                    .errors
                    .push("Could not find home directory".to_string());
                return Ok(report);
            }
        };
        ...
        if !config_dir.exists() {
            if let Err(e) = fs::create_dir_all(&config_dir) {
                report
                    .errors
                    .push(format!("Failed to create config directory: {}", e));
            } else {
