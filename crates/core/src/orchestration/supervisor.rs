...
impl Supervisor {
    ...
    pub async fn run(&mut self) -> Result<String> {
        ...
        while conversation_active && turn_count < max_turns {
            ...
            let agent_arc = self.agents.get(&current_agent_name)
            ...
            let next_target = {
                ...
                for msg in agent.history.iter().rev() {
                        ...
                        for tc in tool_calls {
                            if tc.function.name == "send_message" {
                                ...
                                if let Some(to) = args["to"].as_str() {
                                    ...
                                    last_message = args["message"].as_str().unwrap_or_default().to_string();
                                }
                            } else if tc.function.name == "final_answer" {
                                conversation_active = false;
                                let args: serde_json::Value = serde_json::from_str(&tc.function.arguments).unwrap_or_default();
                                last_message = args["answer"].as_str().unwrap_or_default().to_string();
                            }
                        }
                    }
...
impl pharmakon_common::Tool for TeamMessageTool {
    ...
    fn description(&self) -> &str { "Send a message to another agent in the team." }
...
impl pharmakon_common::Tool for FinalAnswerTool {
    fn name(&self) -> &str { "final_answer" }
    fn description(&self) -> &str { "Provide the final answer to the supervisor and end the task." }
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
            "properties": {
