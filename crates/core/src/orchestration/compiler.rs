...
impl PlanCompiler {
            ...
            PlanNode::Sequence { nodes } => {
                ...
                for child in nodes {
                    ...
                    if let PlanNode::Step { ref tool, ref args, .. } = optimized_child {
                        ...
                        } else if tool == "read_file" || tool == "grep" || tool == "shell" || tool == "codeact" {
                ...
                if filtered_nodes.len() == 1 {
                    ...
                    PlanNode::Sequence { nodes: filtered_nodes }
                }
            }
            ...
            PlanNode::Sequence { nodes } => {
                            ...
                            if current_paths_locked.contains(&path) {
                                ...
                                if !current_parallel_group.is_empty() {
                                    if current_parallel_group.len() == 1 {
                                        ...
                                        optimized_nodes.push(PlanNode::Parallel { nodes: current_parallel_group.clone() });
                                    }
                ...
                if !current_parallel_group.is_empty() {
                    if current_parallel_group.len() == 1 {
                        ...
                        optimized_nodes.push(PlanNode::Parallel { nodes: current_parallel_group });
                    }
                }
                ...
                if optimized_nodes.len() == 1 {
                    ...
                    PlanNode::Sequence { nodes: optimized_nodes }
                }
            }
            PlanNode::Parallel { nodes } => {
                let optimized_nodes = nodes.into_iter().map(|n| self.discover_parallelism(n)).collect();
                PlanNode::Parallel { nodes: optimized_nodes }
            }
            _ => node,
        }
    ...
    fn place_verifications(&self, node: PlanNode) -> PlanNode {
        match node {
            ...
            PlanNode::Parallel { nodes } => {
                let optimized_nodes = nodes.into_iter().map(|n| self.place_verifications(n)).collect();
                PlanNode::Parallel { nodes: optimized_nodes }
            }
            _ => node,
        }
    ...
    pub fn estimate_token_cost(&self, node: &PlanNode) -> u64 {
        match node {
            ...
            PlanNode::Verify { node, .. } => {
                self.estimate_token_cost(node) + 200
            }
            PlanNode::Gate { node, .. } => {
                self.estimate_token_cost(node)
            }
        }
    }
}
