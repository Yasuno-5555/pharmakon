...
impl CognitiveBudget {
    ...
    pub fn spend(&mut self, tokens: u64) {
        ...
        self.shadow_price = if remaining > 0.0 {
            1.0 / remaining
        } else {
            10.0
        };
...
impl ProductionFunction {
    ...
    pub fn estimate_from_genome(
        capability: &str,
        max_success_rate: f32,
        avg_composability: f32,
    ) -> Self {
        Self {
...
impl CrystallizationPriority {
    ...
    pub fn compute(
        ...
        let cost_avoided =
            total_tokens as f64 * (input_cost_per_1k + output_cost_per_1k * 3.0) / 1000.0;
...
impl ModelMarketQuote {
    ...
    pub fn expected_roi(&self, estimated_input_tokens: u64, estimated_output_tokens: u64) -> f64 {
        let total_cost = (estimated_input_tokens as f64 * self.input_cost_per_1k
            + estimated_output_tokens as f64 * self.output_cost_per_1k * 3.0)
            / 1000.0;
        if total_cost <= 0.0 {
...
pub struct TokenReserve {
    ...
    pub emergency_buffer: u64,
    pub burst_quota: u64,
    pub recession_mode: bool,
}
impl TokenReserve {
    pub fn new(total: u64) -> Self {
        Self {
            ...
            reserve_ratio: 0.15,
            emergency_buffer: total / 10,
            burst_quota: total / 20,
            recession_mode: false,
        }
    }
    pub fn withdraw(&mut self, req: u64, prio: f64) -> u64 {
        let avail = self.total_liquidity.saturating_sub(self.emergency_buffer);
        let g = if self.recession_mode {
            (req as f64 * 0.5) as u64
        } else {
            (req as f64 * prio.min(1.0)) as u64
        };
        let a = g.min(avail);
        self.total_liquidity = self.total_liquidity.saturating_sub(a);
        if self.total_liquidity < self.emergency_buffer {
            self.recession_mode = true;
        }
        a
    }
    pub fn deposit(&mut self, u: u64) {
        self.total_liquidity += u;
    }
}

pub struct RecursiveEvpi {
    pub base_evpi: f64,
...
impl RecursiveEvpi {
    pub fn new(base: f64, complexity: f64, success_rate: f64) -> Self {
        let fr = 1.0 - success_rate;
        Self {
            base_evpi: base,
            branching_factor: 1.0 + complexity * 2.0,
            expected_retries: fr * 3.0,
            hallucination_risk: complexity * 0.2 * fr,
        }
    }
    pub fn adjusted_evpi(&self, retry_cost: f64, hall_cost: f64) -> f64 {
        (self.base_evpi - self.expected_retries * retry_cost - self.hallucination_risk * hall_cost)
            .max(0.0)
    }
}

pub struct ContextTax {
    pub scores: Vec<f64>,
    pub rate: f64,
}
impl ContextTax {
    ...
    pub fn tax_value(&self, voi: f64, pollution: f64) -> f64 {
        (voi - self.rate * pollution).max(0.0)
    }
    pub fn record(&mut self, p: f64) {
        self.scores.push(p);
    }
...
pub fn crystallization_variance_adjusted(
    _skill_id: &str,
    avg_saved: f64,
    failure_rate: f32,
    retry_cost: f64,
    ...
    (avg_saved - risk_aversion * failure_rate as f64 * retry_cost).max(0.0)
}

pub struct AdaptiveMarketMaker {
    pub quotes: Vec<ModelMarketQuote>,
    pub rate_limit_probs: std::collections::HashMap<String, f64>,
    pub latency_drifts: std::collections::HashMap<String, f64>,
    pub instability_penalties: std::collections::HashMap<String, f64>,
}
impl AdaptiveMarketMaker {
    pub fn new(q: Vec<ModelMarketQuote>) -> Self {
        let mut r = std::collections::HashMap::new();
        let mut l = std::collections::HashMap::new();
        let mut i = std::collections::HashMap::new();
        for qq in &q {
            r.insert(qq.model_id.clone(), 0.05);
            l.insert(qq.model_id.clone(), 1.0);
            i.insert(qq.model_id.clone(), 0.0);
        }
        Self {
            quotes: q,
            rate_limit_probs: r,
            latency_drifts: l,
            instability_penalties: i,
        }
    }
    pub fn effective_cost(&self, mid: &str, base: f64) -> f64 {
        let rl = self.rate_limit_probs.get(mid).copied().unwrap_or(0.0);
        let ld = self.latency_drifts.get(mid).copied().unwrap_or(1.0);
        let ins = self.instability_penalties.get(mid).copied().unwrap_or(0.0);
        base * (1.0 + rl) * ld + ins
    }
    pub fn speculative_parallelism_value(
        &self,
        cheap: &str,
        expensive: &str,
        it: u64,
        ...
        let cr = self
            .quotes
            .iter()
            .find(|q| q.model_id == cheap)
            .map(|q| q.expected_roi(it, ot))
            .unwrap_or(0.0);
        let er = self
            .quotes
            .iter()
            .find(|q| q.model_id == expensive)
            .map(|q| q.expected_roi(it, ot))
            .unwrap_or(0.0);
        cr * 5.0 * 0.7 > er
    }
...
impl CognitiveMacroState {
    pub fn new() -> Self {
        Self {
            token_reserves: 100_000.0,
            context_inflation: 0.0,
            average_entropy: 0.3,
            model_liquidity: 1.0,
            hallucination_rate: 0.05,
            memory_capital_stock: 0.0,
            memory_depreciation_rate: 0.02,
            active_cognitive_debt: 0.0,
    ...
    pub fn accumulate_memory(&mut self, investment: f64) {
        self.memory_capital_stock =
            (1.0 - self.memory_depreciation_rate) * self.memory_capital_stock + investment;
    }
    ...
    pub fn detect_crisis(&mut self, rate_limit_prob: f64, api_unavailable: bool) {
        ...
        let system_saturated = self.system_cpu_usage > 85.0
            || (self.system_memory_free_mb < 512 && self.system_memory_total_mb > 1024);
    ...
    pub fn crisis_response(&self) -> CrisisPolicy {
        if !self.crisis_mode {
            return CrisisPolicy::Normal;
        }
        if self.token_reserves < 1000.0 {
            return CrisisPolicy::FullOffline;
        }
        if self.context_inflation > 1.5 {
            return CrisisPolicy::AggressiveCompression;
        }
...
impl AgentProfile {
    pub fn new(id: &str, spec: AgentSpecialization) -> Self {
        ...
        Self {
            id: id.to_string(),
            risk_tolerance: risk,
            reasoning_efficiency: eff,
            hallucination_tendency: hall,
            specialization: spec,
            allocated_token_share: share,
        }
    }
...
impl ForwardPlanner {
    pub fn new(discount_factor: f64) -> Self {
        Self {
            expected_future_demand: 0.3,
            expected_rate_limit_prob: 0.1,
            expected_provider_volatility: 0.05,
            discount_factor,
        }
    }
...
pub struct BellmanPlanner {
    pub discount_factor: f64,
    pub value_table: std::collections::HashMap<u64, f64>,
}
impl BellmanPlanner {
    pub fn new(discount_factor: f64) -> Self {
        Self {
    ...
    pub fn endogenous_shadow_price(
        &self,
        reserves: f64,
        expected_future_demand: f64,
        crisis_prob: f64,
        productivity_growth: f64,
    ) -> f64 {
    ...
    pub fn bellman_iteration(
        &mut self,
        budget: u64,
        _complexity: f64,
        production: &ProductionFunction,
    ) -> f64 {
        if let Some(v) = self.value_table.get(&budget) {
            return *v;
        }
...
pub struct KnowledgeCapital {
    pub reusable_trajectories: f64,
    pub crystallized_skills: f64,
    pub memory_embedding_quality: f64,
    pub policy_compression_ratio: f64,
...
impl KnowledgeCapital {
    pub fn new() -> Self {
        Self {
            reusable_trajectories: 0.0,
            crystallized_skills: 0.0,
            memory_embedding_quality: 0.5,
            policy_compression_ratio: 0.3,
            depreciation_rate: 0.01,
            cumulative_investment: 0.0,
        }
    }
    pub fn total(&self) -> f64 {
        self.reusable_trajectories
            + self.crystallized_skills
            + self.memory_embedding_quality
            + self.policy_compression_ratio
    }
    pub fn future_token_savings(&self, annual_token_budget: f64) -> f64 {
        annual_token_budget * self.total() * 0.5
    }
    pub fn invest_research(&mut self, tokens: f64, learning_phi: f64) {
        self.cumulative_investment += tokens;
        self.reusable_trajectories += learning_phi * tokens * 0.001;
        self.crystallized_skills += learning_phi * tokens * 0.0005;
    }
    pub fn depreciate(&mut self) {
        let r = 1.0 - self.depreciation_rate;
        self.reusable_trajectories *= r;
        self.crystallized_skills *= r;
        self.memory_embedding_quality *= r;
        self.policy_compression_ratio *= r;
    }
}

pub struct NonlinearContextInflation {
    pub current_load: f64,
    pub optimal_capacity: f64,
    pub sigmoid_steepness: f64,
}
impl NonlinearContextInflation {
    pub fn new(optimal_capacity: f64) -> Self {
        Self {
            current_load: 0.0,
            optimal_capacity,
    ...
    pub fn update(&mut self, current_tokens: u64) {
        self.current_load = current_tokens as f64;
    }
    pub fn inflation(&self) -> f64 {
        let x = self.current_load - self.optimal_capacity;
        -0.2 + 1.0 / (1.0 + (-self.sigmoid_steepness * x).exp())
    }
    pub fn collapse_risk(&self) -> f64 {
        let inf = self.inflation();
        if inf > 0.8 {
            0.9
        } else if inf > 0.5 {
            0.5
...
impl ProviderPortfolio {
    ...
    pub fn markowitz_allocation(&self, risk_aversion: f64) -> Vec<(String, f64)> {
        ...
        for _ in 0..20 {
            ...
            for i in 0..n {
                ...
                for j in 0..n {
                    let corr = if i == j {
                        ...
                        self.correlated_providers(&self.allocations[i].0, &self.allocations[j].0)
                    };
            ...
            for (wi, w) in weights.iter_mut().enumerate() {
                *w = (*w + 0.1 * grad[wi] * old_weights[wi]).clamp(0.01, 1.0);
            }
        ...
        self.allocations
            .iter()
            .enumerate()
            .map(|(i, (id, _))| (id.clone(), weights[i]))
            .collect()
    }
    pub fn correlated_providers(&self, a: &str, b: &str) -> f64 {
...
pub struct CognitiveLoan {
    pub principal_tokens: u64,
    pub expected_future_savings: f64,
    pub default_probability: f64,
    pub term_turns: u64,
}
impl CognitiveLoan {
    pub fn new(principal: u64, savings: f64, default_prob: f64) -> Self {
        Self {
            principal_tokens: principal,
            expected_future_savings: savings,
            default_probability: default_prob,
            term_turns: 10,
        }
    }
    pub fn expected_value(&self) -> f64 {
        self.expected_future_savings * (1.0 - self.default_probability)
            - self.principal_tokens as f64
    }
    pub fn approve(&self) -> bool {
        self.expected_value() > 0.0 && self.default_probability < 0.3
    }
}

pub struct ExpectationSimulator {
    pub future_token_prices: Vec<f64>,
    pub volatility_estimate: f64,
    pub regime_probabilities: std::collections::HashMap<String, f64>,
...
impl ExpectationSimulator {
    pub fn new() -> Self {
        ...
        Self {
            future_token_prices: Vec::new(),
            volatility_estimate: 0.1,
            regime_probabilities: std::collections::HashMap::new(),
    ...
    pub fn expected_future_price(&self, horizon: usize) -> f64 {
        if self.future_token_prices.is_empty() {
            return 0.01;
        }
...
impl RegimeSwitcher {
    pub fn new() -> Self {
        ...
        Self {
            current: MacroRegime::Normal,
            transition_matrix: tm,
            regime_durations: std::collections::HashMap::new(),
        }
    }
    pub fn update(
        &mut self,
        macro_state: &CognitiveMacroState,
        rate_limit_prob: f64,
        api_unavailable: bool,
    ) {
        let row = self.transition_matrix[self.current as usize];
        let mut adjusted = row;
        if macro_state.context_inflation > 0.5 {
            adjusted[1] += 0.2;
            adjusted[0] -= 0.2;
        }
        if macro_state.crisis_mode || rate_limit_prob > 0.5 {
            adjusted[2] += 0.3;
            adjusted[0] -= 0.3;
        }
        if api_unavailable {
        ...
        for (i, p) in adjusted.iter().enumerate() {
            ...
            if r < cum {
                self.current = match i {
                    0 => MacroRegime::Normal,
                    1 => MacroRegime::Congestion,
                    2 => MacroRegime::Crisis,
                    _ => MacroRegime::Offline,
                };
                break;
            }
        }
    ...
    pub fn policy(&self) -> RegimePolicy {
        match self.current {
            MacroRegime::Normal => RegimePolicy {
                max_tokens: 8192,
                max_retries: 3,
                prefer_cache: false,
            },
            MacroRegime::Congestion => RegimePolicy {
                max_tokens: 4096,
                max_retries: 2,
                prefer_cache: true,
            },
            MacroRegime::Crisis => RegimePolicy {
                max_tokens: 2048,
                max_retries: 1,
                prefer_cache: true,
            },
            MacroRegime::Offline => RegimePolicy {
                max_tokens: 512,
                max_retries: 0,
                prefer_cache: true,
            },
...
pub struct EndogenousHallucinationModel {
    pub base_rate: f64,
...
impl EndogenousHallucinationModel {
    pub fn new() -> Self {
        Self {
            ...
            entropy_sensitivity: 0.10,
            fatigue_factor: 0.02,
        }
    }
    pub fn rate(&self, inflation: f64, entropy: f64, turns: u64) -> f64 {
        (self.base_rate
            + self.inflation_sensitivity * inflation.max(0.0)
            + self.entropy_sensitivity * entropy
            + self.fatigue_factor * turns as f64)
            .clamp(0.0, 0.95)
    }
}
...
impl GeneralEquilibrium {
    ...
    pub fn add(&mut self, a: AgentProfile) {
        self.agents.push(a);
    }
        ...
        for _ in 0..iters {
            ...
            let demand: f64 = self
                ...
                .map(|a| {
                    ...
                    (base + elasticity * price_gap * base)
                        .max(0.0)
                        .min(self.total_supply as f64 * 2.0)
                })
                .sum();
...
impl TokenDerivative {
    pub fn new(kind: DerivativeKind, strike: f64, qty: u64, expiry: u64) -> Self {
        Self {
            kind,
            strike_price: strike,
            quantity: qty,
    ...
    pub fn payoff(&self, current_price: f64) -> f64 {
        match self.kind {
            ...
            DerivativeKind::Option => {
                ((current_price - self.strike_price).max(0.0)) * self.quantity as f64
            }
        }
    }
}
