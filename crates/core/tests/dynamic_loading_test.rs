...
fn create_default_model() -> Arc<dyn pharmakon_core::model::AgentModel> {
    let api_key = std::env::var("GEMINI_API_KEY").unwrap_or_else(|_| "dummy_key_for_default".to_string());
...
async fn test_dynamic_agent_loading() {
    ...
    if let Ok(key) = std::env::var("GEMINI_API_KEY") {
        ...
        println!("Warning: GEMINI_API_KEY not set. Test might fail if it needs to contact the real API.");
    }
    ...
    // Check model
    assert!(gemini_agent.model.name().contains("gemini-2.5-flash"), "Gemini agent should have the specified Gemini model. Found: {}", gemini_agent.model.name());

    // Check tools
    let gemini_tools: Vec<String> = gemini_agent.tools.iter().map(|t| t.name().to_string()).collect();
    assert_eq!(gemini_tools.len(), 1, "Gemini agent should have 1 tool, but found {}. Tools: {:?}", gemini_tools.len(), gemini_tools);
    assert!(gemini_tools.contains(&"web_fetch".to_string()), "Gemini agent should have the 'web_fetch' tool");
    println!("'gemini_researcher' agent loaded correctly.");

    // 4. Test the OpenAI agent (which should fall back to the default model)
    let openai_agent_handle = router.get_agent("openai_coder_fallback").await.expect("Failed to get openai_coder_fallback agent");
    let openai_agent = openai_agent_handle.lock().await;

    // Check model (should be the default model)
    assert!(openai_agent.model.name().contains("gemini-2.5-flash"), "OpenAI agent should fall back to the default Gemini model. Found: {}", openai_agent.model.name());

    // Check tools
    let openai_tools: Vec<String> = openai_agent.tools.iter().map(|t| t.name().to_string()).collect();
    assert_eq!(openai_tools.len(), 2, "OpenAI agent should have 2 tools");
    assert!(openai_tools.contains(&"shell".to_string()), "OpenAI agent should have the 'shell' tool");
    assert!(openai_tools.contains(&"read_file".to_string()), "OpenAI agent should have the 'read_file' tool");
