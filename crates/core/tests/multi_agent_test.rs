...
impl AgentModel for MultiAgentMockModel {
    async fn complete(&self, request: CompletionRequest) -> AgentResult<CompletionResponse> {
        ...
        if content.contains("Research latest AI trends") {
             Ok(CompletionResponse {
                 content: Some(MessageContent::Text("I'll ask the researcher to look into this.".to_string())),
            ...
            Ok(CompletionResponse {
                content: Some(MessageContent::Text("I found that Generative AI is huge in 2024.".to_string())),
                tool_calls: Some(vec![ToolCall {
                    id: "call_2".to_string(),
                    r#type: "function".to_string(),
                    function: FunctionCall {
            ...
            Ok(CompletionResponse {
                content: Some(MessageContent::Text("Generative AI is the top trend for 2024.".to_string())),
                tool_calls: Some(vec![ToolCall {
                    id: "call_3".to_string(),
                    r#type: "function".to_string(),
                    function: FunctionCall {
    ...
    fn name(&self) -> &str { "multi-agent-mock" }
...
async fn test_multi_agent_collaboration() {
    ...
    let manager = Arc::new(Mutex::new(Agent::new(model.clone(), "manager-session".to_string())));
    let researcher = Arc::new(Mutex::new(Agent::new(model.clone(), "researcher-session".to_string())));
    
    manager.lock().await.add_tool(Arc::new(pharmakon_core::orchestration::TeamMessageTool { from: "Manager".to_string() }));
    manager.lock().await.add_tool(Arc::new(pharmakon_core::orchestration::FinalAnswerTool));
    researcher.lock().await.add_tool(Arc::new(pharmakon_core::orchestration::TeamMessageTool { from: "Researcher".to_string() }));

    let mut supervisor = Supervisor::new("Research latest AI trends".to_string(), "Manager".to_string());
