...
impl SystemTelemetry {
    ...
    pub fn get_summary_24h(&self) -> String {
        ...
        let avg_cpu: f32 = self.history.iter().map(|s| s.cpu_usage).sum::<f32>() / self.history.len() as f32;
        let max_cpu: f32 = self.history.iter().map(|s| s.cpu_usage).fold(0.0, f32::max);
        
        format!("Last 24h Summary: Avg CPU {:.1}%, Max CPU {:.1}%. Sample count: {}", avg_cpu, max_cpu, self.history.len())
    }
}
