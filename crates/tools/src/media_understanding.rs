...
use pharmakon_common::{
    AgentError, AgentModel, AgentResult, CompletionRequest, ContentPart, ImageUrl, Message,
    MessageContent, Tool,
};
...
impl Tool for MediaUnderstandingTool {
    ...
    fn description(&self) -> &str {
        "Describe the content of an image file. Use this to 'see' what is in a media file."
    }
    ...
    async fn call(&self, args: Value) -> AgentResult<String> {
        ...
        let bytes = fs::read(path)
            .map_err(|e| AgentError(format!("Failed to read file {}: {}", path, e)))?;
        ...
        log::info!(
            "MediaUnderstandingTool: Sending vision request for {}...",
            path
        );
