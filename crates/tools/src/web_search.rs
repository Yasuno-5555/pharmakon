...
use pharmakon_common::{Tool, AgentResult, AgentError, SecretStore};
...
impl Tool for BraveSearchTool {
    ...
    fn description(&self) -> &str { "Search the web for current information. Returns a list of results with snippets." }
    ...
    async fn call(&self, args: Value) -> AgentResult<String> {
        ...
        let api_key = secret_store.get_secret("BRAVE_API_KEY")
            .or_else(|_| std::env::var("BRAVE_API_KEY"))
            .map_err(|_| AgentError("BRAVE_API_KEY not found. Please set it in secrets or environment.".to_string()))?;
        ...
        if !response.status().is_success() {
            return Err(AgentError(format!("Brave Search API error: {}", response.status())));
        }
