...
impl Tool for TemporalAwarenessTool {
    ...
    async fn call(&self, args: Value) -> AgentResult<String> {
        ...
        let last_modified = String::from_utf8_lossy(&last_mod_cmd.stdout)
            .trim()
            .to_string();
...
impl Tool for FailurePredictionTool {
    ...
    async fn call(&self, args: Value) -> AgentResult<String> {
        ...
        let avg_indent = if lines_of_code > 0 {
            avg_indent_acc as f64 / lines_of_code as f64
        } else {
            0.0
        };
        ...
        let bug_fix_count = if bug_output.trim().is_empty() {
            0
        } else {
            bug_output.lines().count()
        };
        ...
        let total_commits = if total_output.trim().is_empty() {
            0
        } else {
            total_output.lines().count()
        };
        ...
        if max_indent > 24 {
            risk_score += 20.0;
        } else if max_indent > 16 {
            risk_score += 10.0;
        }

        // History contribution (max 40)
        if defect_density > 50.0 {
            risk_score += 40.0;
        } else if defect_density > 30.0 {
            risk_score += 25.0;
        } else if defect_density > 10.0 {
            risk_score += 10.0;
        }

        // Safety Hotspot penalty contribution (max 20)
        let hotspot_penalty = (unwrap_count * 2)
            + (unsafe_count * 5)
            + (panic_count * 3)
            + (deep_nest_loops * 4)
            + (clone_count / 3);
        ...
        report.push_str(&format!(
            "- **Max Indentation Depth**: {} spaces\n",
            max_indent
        ));
        report.push_str(&format!(
            "- **Avg Indentation Depth**: {:.1} spaces\n",
            avg_indent
        ));
        report.push_str(&format!(
        ...
        report.push_str(&format!(
            "- **Unchecked Panickers (`.unwrap()`/`.expect()`))**: {}\n",
            unwrap_count
        ));
        report.push_str(&format!(
        ...
        report.push_str(&format!(
            "- **Explicit Panics (`panic!()`/`todo!()`)**: {}\n",
            panic_count
        ));
        report.push_str(&format!(
            "- **Heap Allocating Clones (`.clone()`)**: {}\n\n",
            clone_count
        ));
        ...
        if risk_score >= 45.0 {
            ...
            report.push_str("\n> [!NOTE]\n");
            report.push_str(
                "> This file appears relatively safe and maintains high code hygiene standards.",
            );
        }
        ...
        match scan_type {
            "clippy_perf" => {
                ...
                if issues.is_empty() {
                    ...
                    let mut report =
                        String::from("### Performance Optimization Opportunities (Clippy)\n\n");
                    for issue in issues.iter().take(20) {
                        report.push_str(&format!("- `{}`\n", issue));
                    }
                    if issues.len() > 20 {
                        report.push_str(&format!(
                            "\n...and {} more issues. Please fix these first.",
                            issues.len() - 20
                        ));
                    }
                    Ok(report)
            ...
            "hotspot_analysis" => {
                ...
                let mut report =
                    String::from("### Hotspot Analysis (Potential Unnecessary Clones)\n");
        ...
        match action {
            "record" => {
                let context = args["context"]
                    .as_str()
                    .ok_or_else(|| AgentError("Missing 'context'".into()))?;
                let approach = args["approach"]
                    .as_str()
                    .ok_or_else(|| AgentError("Missing 'approach'".into()))?;
                let reason = args["reason"]
                    .as_str()
                    .ok_or_else(|| AgentError("Missing 'reason'".into()))?;
                ...
                Self::save_regrets(&regrets)
                    .map_err(|e| AgentError(format!("Failed to save: {}", e)))?;
                Ok(
                    "Regret recorded successfully. This approach will be avoided in the future."
                        .into(),
                )
            }
            "query" => {
                ...
                if !filter_ctx.is_empty() {
                    ...
                    for r in &regrets {
                        ...
                        for term in &query_terms {
                            if context_lower.contains(term) {
                                score += 10.0;
                            }
                            if approach_lower.contains(term) {
                                score += 3.0;
                            }
                            if reason_lower.contains(term) {
                                score += 2.0;
                            }
                        }

                        // Large bonus for exact phrase match
                        if context_lower.contains(&filter_ctx.to_lowercase()) {
                            score += 20.0;
                        }
                ...
                if scored_regrets.is_empty() {
                    return Ok(format!(
                        "No regrets matched the query context '{}'.",
                        filter_ctx
                    ));
                }
                ...
                output.push_str(&format!(
                    "### Known Regrets & Avoided Approaches (Semantic Search Results for '{}')\n\n",
                    filter_ctx
                ));

                for (i, (score, r)) in scored_regrets.iter().enumerate() {
                    output.push_str(&format!(
                        "#### Regret #{} (Relevance Score: {:.1})\n",
                        i + 1,
                        score
                    ));
            ...
            _ => Err(AgentError(
                "Invalid action. Must be 'record' or 'query'.".into(),
            )),
        }
    }
}
