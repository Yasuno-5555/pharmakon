...
impl Tool for ScreenshotTool {
    ...
    fn description(&self) -> &str { "Capture a screenshot of the main display." }
    ...
    async fn call(&self, args: Value) -> AgentResult<String> {
        ...
        let screens = Screen::all().map_err(|e| AgentError(format!("Failed to get screens: {}", e)))?;
        
        let screen = screens.get(display_id)
            .or_else(|| screens.get(0))
            .ok_or_else(|| AgentError("No screens found".to_string()))?;

        let image = screen.capture().map_err(|e| AgentError(format!("Failed to capture screen: {}", e)))?;
        
        let home = dirs::home_dir().ok_or_else(|| AgentError("Could not find home directory".to_string()))?;
        ...
        image.save(&path).map_err(|e| AgentError(format!("Failed to save screenshot: {}", e)))?;
...
impl Tool for CameraTool {
    ...
    fn description(&self) -> &str { "Capture a frame from the default camera." }
    fn parameters(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
    ...
    async fn call(&self, args: Value) -> AgentResult<String> {
        ...
        camera.open_stream().map_err(|e| AgentError(format!("Failed to open camera stream: {}", e)))?;
        let frame = camera.frame().map_err(|e| AgentError(format!("Failed to capture camera frame: {}", e)))?;
        let decoded = frame.decode_image::<RgbFormat>().map_err(|e| AgentError(format!("Failed to decode frame: {}", e)))?;
        ...
        decoded.save(&path).map_err(|e| AgentError(format!("Failed to save camera frame: {}", e)))?;
