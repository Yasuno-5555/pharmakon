...
struct LocalToolContext {
    #[serde(default)]
    tool_notes: HashMap<String, LocalToolNote>,
    #[serde(default)]
    general_heuristics: Vec<String>,
}
...
impl Tool for UpdateContextTool {
    ...
    async fn call(&self, args: Value) -> AgentResult<String> {
        let target = args["target"]
            .as_str()
            .ok_or_else(|| AgentError("Missing target".to_string()))?;
        ...
        if !base_dir.exists() {
            fs::create_dir_all(&base_dir)
                .map_err(|e| AgentError(format!("Failed to create context dir: {}", e)))?;
        }
        ...
        match target {
            "identity" => {
                ...
                match action {
                    ...
                    "add_trait" => {
                        if !ctx.traits.contains(&value.to_string()) {
                            ctx.traits.push(value.to_string());
                        }
                    }
                    "add_core_directive" => {
                        if !ctx.core_directives.contains(&value.to_string()) {
                            ctx.core_directives.push(value.to_string());
                        }
                    }
                    _ => {
                        return Err(AgentError(format!(
                            "Invalid action for identity context: {}",
                            action
                        )));
                    }
                }
                let yaml_str = serde_yaml::to_string(&ctx)
                    .map_err(|e| AgentError(format!("YAML serialization error: {}", e)))?;
            ...
            "user" => {
                ...
                match action {
                    ...
                    "add_user_preference" => {
                        if key.is_empty() {
                            return Err(AgentError("Key is required".to_string()));
                        }
                    ...
                    _ => {
                        return Err(AgentError(format!(
                            "Invalid action for user context: {}",
                            action
                        )));
                    }
                }
            ...
            "tools" => {
                ...
                match action {
                    "add_general_heuristic" => {
                        if !ctx.general_heuristics.contains(&value.to_string()) {
                            ctx.general_heuristics.push(value.to_string());
                        }
                    }
                    "add_tool_guideline" => {
                        if key.is_empty() {
                            return Err(AgentError("Key required".to_string()));
                        }
                        let note = ctx.tool_notes.entry(key.to_string()).or_default();
                        if !note.usage_guidelines.contains(&value.to_string()) {
                            note.usage_guidelines.push(value.to_string());
                        }
                    }
                    "add_tool_quirk" => {
                        if key.is_empty() {
                        ...
                        let note = ctx.tool_notes.entry(key.to_string()).or_default();
                        if !note.known_quirks.contains(&value.to_string()) {
                            note.known_quirks.push(value.to_string());
                        }
                    }
                    _ => {
                        return Err(AgentError(format!(
                            "Invalid action for tools context: {}",
                            action
                        )));
                    }
                }
