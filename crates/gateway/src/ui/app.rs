...
impl AppData {
    pub fn new(
        ...
        Self {
            ...
            cognitive_timeline: vec![TimelineEvent {
                timestamp: chrono::Utc::now().format("%H:%M:%S").to_string(),
                event: "IDE started".into(),
        ...
        while let Ok(event) = self.event_rx.try_recv() {
            match event {
                ...
                UiEvent::ToolCall { name, args } => {
                    ...
                    if self.tool_trace.len() > 100 {
                        self.tool_trace.remove(0);
                    }
                }
                UiEvent::ToolResult(r) => {
                    let short = if r.len() > 120 {
                        format!("{}...", &r[..117])
                    } else {
                        r
                    };
                ...
                UiEvent::Error(msg) => {
                    self.system_logs.push(format!("ERR: {}", msg));
                }
                UiEvent::ModelList(m) => {
                    self.model_list = m;
                }
                UiEvent::ModelSwitched(id) => {
                    self.current_model = id;
                }
                UiEvent::TokenUsage { tokens, cost } => {
                    self.token_count = tokens;
                    self.total_cost = cost;
                }
                UiEvent::GatewayStatus { uptime, memory } => {
                    self.uptime_secs = uptime;
                    self.memory_mb = memory / 1024 / 1024;
                }
                UiEvent::McpStats(_s) => {}
                UiEvent::ToolList(t) => {
                    self.tools = t;
                ...
                UiEvent::OrchestrationState(s) => {
                    self.active_swarms = s;
                }
                UiEvent::GraphUpdate(r) => {
                    self.graph_relations = r;
                }
                UiEvent::SettingsUpdate(_) => {}
                UiEvent::CronJobList(j) => {
                    self.cron_jobs = j;
                }
                UiEvent::PlanDagUpdate(dag) => {
                    self.plan_dag = dag;
                }
                UiEvent::TimelineEvent(ev) => {
                    self.cognitive_timeline.push(ev);
                    if self.cognitive_timeline.len() > 50 {
                        self.cognitive_timeline.remove(0);
                    }
                }
                UiEvent::SnapshotCreated(id) => {
                ...
                UiEvent::FileTreeLoaded(nodes) => {
                    ...
                    self.system_logs
                        .push("📂 Workspace file tree loaded successfully".into());
                }
            }
    ...
    pub fn send_message(&mut self) {
        ...
        tokio::spawn(async move {
            let _ = agent.chat(&workspace_hint).await;
        });
    }
    ...
    pub fn rollback(&mut self) {
        if let Some(ref snap_id) = self.last_snapshot_id.clone() {
            ...
            let agent = self.agent.clone();
            tokio::spawn(async move {
                let events = agent.event_log.events_since(0u64).await;
                if !events.is_empty() {
            ...
            let ev = TimelineEvent {
                timestamp: chrono::Utc::now().format("%H:%M:%S").to_string(),
                event: format!("Rollback → {}", &sid[..sid.len().min(8)]),
