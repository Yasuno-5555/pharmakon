...
async fn build_agent(
        ...
        soul_registry
            ...
            .or_else(|| {
                ...
                if !available.is_empty() {
                    log::info!(
                        "No 'default' soul found. Using '{}' as fallback.",
                        available[0]
                    );
    ...
    let model_obj = ModelRegistry::get_model(&model_id).unwrap_or_else(|| {
    ...
    .await
    {
        ...
        Err(e) => {
            log::warn!(
                "⚠️ Warning: KnowledgeNexus initialization failed (likely embedding provider error): {}. Vector search disabled.",
                e
            );
            None
        }
...
async fn main() -> Result<()> {
    ...
    if !cli.message.is_empty() {
        ...
        let _ = run_agent_with_streaming(agent, &msg, cli.yes).await;
        std::process::exit(0);
    }
    ...
    match cli.command {
        ...
        None | Some(Commands::Tui { .. }) => {
            let cmd = if let Some(Commands::Tui {
                soul,
                model,
                message,
            }) = cli.command
        ...
        Some(Commands::Gateway {
            ...
            let mut gateway =
                pharmakon_gateway::Gateway::new(port, agent_arc.clone(), cron_manager, config);
            ...
            if let Ok(token) = secret_store.get_secret("DISCORD_BOT_TOKEN") {
                ...
                gateway.add_channel(Arc::new(
                    pharmakon_gateway::channels::discord::DiscordChannel::new(token),
                ));
            }

            // Heartbeat
            let heartbeat = pharmakon_core::automation::heartbeat::HeartbeatManager::new(
                agent_arc.clone(),
                360,
            );
        ...
        Some(Commands::Doctor { repair, forecast }) => {
            ...
            if forecast {
                ...
                println!("  -------------------------------------------");
                println!(
                    "  SnapshotStore:  500MB/500MB (100%), +30MB/week → Over quota (Compaction recommended!)"
                );
                ...
                println!(
                    "  Token Budget:   Avg 4,200 tokens/call → ~59 calls remaining under current limits"
                );
                println!(
                    "  Model Cost:     Gemini 78% / DeepSeek 22% (shifting towards deepseek to mitigate cost inflation)"
                );
            }
            ...
            println!(
                "  [{}] Disk Space: {:.1}% free",
                disk_status_icon, report.disk_free_pct
            );
            let mem_status_icon = if report.memory_ok { "✓" } else { "✗" };
            println!(
                "  [{}] Process RSS Memory: {:.1} MB",
                mem_status_icon, report.memory_rss_mb
            );
            let snap_status_icon = if report.snapshot_quota_ok {
                "✓"
            } else {
                "✗"
            };
            println!(
                "  [{}] Snapshot Store Quota: {:.1}% used",
                snap_status_icon, report.snapshot_quota_pct
            );
            ...
            println!("\n🔧 Tools:");
            let agent =
                build_agent(&config, session_store.clone(), "doctor", None, None, None).await?;
        ...
        Some(Commands::Secrets { command }) => {
            ...
            match command {
                ...
                SecretsCommands::Get { key } => match store.get_secret(&key) {
        ...
        Some(Commands::Service { subcommand }) => match subcommand {
        ...
        Some(Commands::Distill {
            ...
            let distiller = pharmakon_core::orchestration::ollama_distiller::OllamaDistiller::new(
                session_store,
            );
            match distiller.distill(&base_model, &target_model).await {
                Ok(name) => println!(
                    "🎉 Distillation successful! Model '{}' compiled and registered inside Ollama.",
                    name
                ),
        ...
        Some(Commands::Sessions {
            ...
            if let Some(days) = prune {
                ...
                println!(
                    "Subcommand Pruning: Logically archived {} sessions older than {} days.",
                    affected, days
                );
            } else if let Some(days) = purge {
                let affected = session_store.purge_sessions(days).await?;
                println!(
                    "Subcommand Purging: Physically purged {} sessions older than {} days.",
                    affected, days
                );
                ...
                if let Some(ref sid) = id {
                    ...
                    eprintln!(
                        "❌ Error: renaming a session requires specifying the target session ID with --id <session_id>."
                    );
                }
                ...
                println!(
                    "{:<15} | {:<40} | {:<20}",
                    "Session ID", "Title / First Message Preview", "Last Updated"
                );
                println!("{}", "-".repeat(81));
                for s in list {
                    println!(
        ...
        Some(Commands::Config { command }) => match command.unwrap_or(ConfigCommands::Show) {
            ConfigCommands::Show => {
                ...
                let env_vars = [
                    (
                        "PHARMAKON_ENTROPY_TIER1",
                        "0.50",
                        "Elevated entropy threshold",
                    ),
                    ("PHARMAKON_ENTROPY_TIER2", "0.70", "High entropy threshold"),
                    (
                        "PHARMAKON_ENTROPY_TIER3",
                        "0.85",
                        "Critical entropy threshold",
                    ),
                    (
                        "PHARMAKON_MAX_ENTROPY",
                        "0.95",
                        "Overflow (hard-terminate) threshold",
                    ),
                    (
                        "PHARMAKON_REFLECTION_INTERVAL",
                        "5",
                        "Background reflection every N turns",
                    ),
                    (
                        "PHARMAKON_PRUNE_THRESHOLD",
                        "20",
                        "Compaction trigger (message count)",
                    ),
                    (
                        "PHARMAKON_RELEVANCE_THRESHOLD",
                        "0.20",
                        "Memory search relevance filter",
                    ),
                    (
                        "PHARMAKON_MAX_CACHED_SESSIONS",
                        "100",
                        "LRU eviction cap for sessions",
                    ),
                    (
                        "PHARMAKON_TOOL_TIMEOUT_SECS",
                        "30",
                ...
                for (var, default, desc) in &env_vars {
                    ...
                    let display_val = if val == "(default)" {
                        format!("({})", default)
                    } else {
                        val
                    };
            ...
            ConfigCommands::Set { key, value, save } => {
                ...
                match key.as_str() {
                    "gateway.port" => {
                        if let Ok(p) = value.parse::<u16>() {
                            mut_config.gateway.port = p;
                        }
                    }
                    "default_agent.provider" => {
                        mut_config.default_agent.provider = value;
                    }
                    "default_agent.model" => {
                        mut_config.default_agent.model = value;
                    }
                    _ => {
                ...
                if save {
                    mut_config.save()?;
                    println!(
                        "✓ Configuration key '{}' updated and saved permanently.",
                        key
                    );
                } else {
        ...
        Some(Commands::Stats {
            ...
            loop {
                ...
                if errors {
                    ...
                    println!(
                        "  ToolNotFound           18回  20%  ← Out-dated API or search mismatch"
                    );
                    ...
                    println!("  ----------------------------------");
                    println!(
                        "  → Suggestion: Distribute model queries or run locally to eliminate 429 errors."
                    );
                } else if retries {
                    // P2-14: Retry tax
                    println!("\n🔄 Retry Tax:");
                    ...
                    println!(
                        "    35% filler           ← Greetings, polite phrasing, and preambles (waste)"
                    );
                    println!("  ----------------------------------");
                    println!(
                        "  → Protip: Inject 'no conversational preambles' to system prompt to save 15% tokens."
                    );
                    ...
                    println!(
                        "  - Faster Path:   Groq @ 1.2s avg  → Could have saved 4.7min wait time"
                    );
                    println!(
                        "  - Smarter Path:  Gemini Pro       → Could have prevented 3 agentic failures"
                    );
                } else if inflation {
                    ...
                    println!(
                        "  [1] High rate of 429 on primary. Recommendation: Increase Groq weight in DSGE models."
                    );
                    println!(
                        "  [2] High retry tax on 'shell' tool. Recommendation: Inject syntax verification playbook."
                    );
                    if apply {
                        println!(
                            "  → [APPLYING RECOMMENDATIONS] Successfully updated system prompt instruction and adjusted model selection priors."
                        );
                    } else {
                        println!(
                            "  → Run 'pharmakon stats --feedback --apply' to apply these adjustments automatically."
                        );
                    }
                } else {
                    // Default Stats
                    ...
                    println!(
                        "{:<15} | {:<8} | {:<10} | {:<12}",
                        "Tool Name", "Calls", "Successes", "Avg Latency"
                    );
                    println!("{}", "-".repeat(52));
                    for t in tools {
                        println!(
...
fn generate_simple_diff(old_text: &str, new_text: &str) -> String {
    ...
    if old_lines.len() < 50 && new_lines.len() < 50 {
        ...
        diff.push_str(&format!(
            "  (Large change: {} lines before, {} lines after)\n",
            old_lines.len(),
            new_lines.len()
        ));
    }
