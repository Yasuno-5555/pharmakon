...
use pharmakon_tools::{ShellTool, FileReadTool, WebFetchTool, BraveSearchTool};
use pharmakon_common::{Config, SecretStore, AgentResult};
...
async fn main() -> Result<()> {
    ...
    let cmd = cli.command.unwrap_or(Commands::Tui { soul: None, model: None });

    match cmd {
        Commands::Gateway { subcommand, port, verbose: _, soul: soul_path } => {
            ...
            let model_id = format!("{}/{}", config.default_agent.provider, config.default_agent.model);
            let model_obj = ModelRegistry::get_model(&model_id).expect("Default model must be available");
            ...
            let fact_memory = Arc::new(Mutex::new(pharmakon_memory::fact_memory::FactMemory::new()?));
            ...
            agent.set_soul(soul); agent.add_tool(Arc::new(ShellTool)); agent.add_tool(Arc::new(FileReadTool)); agent.add_tool(Arc::new(WebFetchTool::new())); agent.add_tool(Arc::new(BraveSearchTool::new("".to_string())));
            
            let gateway = pharmakon_gateway::Gateway::new(actual_port, Arc::new(Mutex::new(agent)), cron_manager, config);
            gateway.run().await?;
        }
        Commands::Agent { message, session, soul: soul_path, provider, model, trajectory: _ } => {
            let soul = if let Some(path) = soul_path {
                Soul::load_from_file(path)?
            } else {
                Soul::default_soul()
            ...
            let model_obj = ModelRegistry::get_model(&model_id).expect("Selected model not available. Check your API keys.");
            ...
            impl pharmakon_common::Tool for SelfDiagnosticTool {
                ...
                fn description(&self) -> &str { "Report current system health, performance, and resource usage." }
